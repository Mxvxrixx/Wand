...
const CategorySectionBase = ({
  ...
  const { toggleCount, enabledCount } = useMemo(() => countToggles(group.cheats, values), [group.cheats, values]);
  ...
  return (
    <section className="mb-2.5 overflow-hidden rounded-[14px] border border-white/10 bg-white/[0.035] shadow-[inset_0_1px_0_rgba(255,255,255,.05)] backdrop-blur-2xl">
      ...
      <div id={panelId} className={cn('grid transition-[grid-template-rows] duration-300', open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]')}>
...
function countToggles(cheats: CheatSchema[], values: Record<string, unknown>): { toggleCount: number; enabledCount: number } {
  let toggleCount = 0;
  let enabledCount = 0;
