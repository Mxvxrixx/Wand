...
export const STEPPER_SHELL_CLASS =
  'grid h-[38px] w-full items-stretch overflow-hidden rounded-[10px] border border-white/10 bg-white/[0.055] shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-xl';
...
export const SliderTrack = ({ min, max, step, value, label, disabled, onInput }: SliderTrackProps) => {
...
export const SliderReadout = ({ value, min, max, step, postfix, label, disabled, onInput }: SliderReadoutProps) => {
  return (
    <>
      <div className="mb-1 flex justify-end font-mono text-[12.5px] tabular-nums text-(--deck-accent)">
        {formatNumber(value, step)}{postfix}
      </div>
      <SliderTrack disabled={disabled} label={label} max={max} min={min} step={step} value={value} onInput={onInput} />
    </>
  );
};
