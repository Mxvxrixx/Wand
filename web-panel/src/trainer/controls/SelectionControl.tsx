...
export const SelectionControl = ({ cheat, value, disabled, onChange }: ControlInternalProps) => {
  ...
  const selectedOption = options.find((option) => isSameOption(option.value, value ?? options[0].value)) ?? options[0];
  const handleToggle = () => {
    if (disabled) {
      return;
    }
  ...
  return (
    <div ref={rootRef} className="w-full space-y-1.5">
      ...
      {open ? (
        <div role="listbox" aria-label={cheat.name} className="overflow-hidden rounded-[10px] border border-white/10 bg-white/[0.055] p-1 shadow-[0_18px_40px_rgba(0,0,0,0.36),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl">
          {options.map((option) => {
