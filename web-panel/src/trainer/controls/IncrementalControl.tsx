...
export const IncrementalControl = ({ cheat, value, disabled, onChange }: ControlInternalProps) => {
  const options = useMemo(() => (cheat.args.options ?? []).map(resolveOption), [cheat.args.options]);
  if (options.length === 0) {
  ...
  return (
    <div className={cn(STEPPER_SHELL_CLASS, INCREMENTAL_STEP_GRID)}>
      <StepButton border="right" disabled={disabled || !previous} icon="chevron-left" label={`Previous ${cheat.name}`} onClick={() => previous && onChange(previous.value)} />
      ...
      <StepButton border="left" disabled={disabled || !next} icon="chevron-right" label={`Next ${cheat.name}`} onClick={() => next && onChange(next.value)} />
    </div>
  );
};
