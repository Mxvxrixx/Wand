...
import { STEPPER_SHELL_CLASS, StepButton, type ControlInternalProps } from './shared';
...
export const NumberControl = ({ cheat, value, disabled, onChange }: ControlInternalProps) => {
  ...
  const decrement = () => commit(snapToStep(Math.max(cheat.args.min ?? Number.NEGATIVE_INFINITY, currentValue - step), step));
  const increment = () => commit(snapToStep(Math.min(cheat.args.max ?? Number.POSITIVE_INFINITY, currentValue + step), step));

  return (
    <div className={cn(STEPPER_SHELL_CLASS, NUMBER_STEP_GRID)}>
      <StepButton border="right" disabled={disabled} icon="minus" label={`Decrease ${cheat.name}`} onClick={decrement} />
      <input
        type="text"
        inputMode="decimal"
      ...
      />
      <StepButton border="left" disabled={disabled} icon="plus" label={`Increase ${cheat.name}`} onClick={increment} />
    </div>
  );
};
